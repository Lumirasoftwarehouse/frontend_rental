    ...
    <div id="content-wrapper" class="d-flex flex-column">
      <div id="content">
        ...
        <div class="container-fluid mt-4">
          <h1 class="h3 mb-0 text-gray-800 text-center">Manajemen Pemesanan</h1>
      ...
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            />
            <i style="color: red; font-size: 10px">* khusus weekend akan dikenakan biaya tambahan Rp. 50.000</i>
            <br>
            ...
            <select v-model="form.menu_id" class="form-control mb-3">
              ...
              <option
                ...
                :value="menu.id"
              >
                {{ menu.name }} - {{ menu.jenis }} - {{menu.jenis === 'PS 4' ? `Rp. 30.000 / sesi` : 'Rp. 40.000 / sesi'}}
              </option>
            </select>
          </div>
...
export default {
  ...
  methods: {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_API_ENDPOINT}/menu/available`, {
    ...
    async submitForm() {
      ...
      try {
        const response = await axios.post(`${import.meta.env.VITE_API_ENDPOINT}${endpoint}`, this.form, {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("token")}`,
          },
        });
        ...
        if (response.data && response.data.data && response.data.data.redirect_url) {
          ...
          Swal.fire("Success!", this.isEdit ? "Pemesanan diperbarui!" : "Pemesanan ditambahkan!", "success");
          this.hideModal();
          this.fetchPemesanan();
        }
      } catch (error) {
